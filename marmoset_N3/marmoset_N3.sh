...
# ARG_OPTIONAL_SINGLE([distance],[],[Initial distance for correction],[70])
...
# ARG_OPTIONAL_SINGLE([isostep],[],[Isotropic resampling resolution in mm for N3],[0.72])
# ARG_OPTIONAL_BOOLEAN([big-misorientation],[],[Input file is very misoriented])

# ARG_OPTIONAL_SINGLE([model],[],[Reference model used for determining FOV and mask],[templates_original/template_T2w_180_pad_renorm_clamp.mnc])
# ARG_OPTIONAL_SINGLE([model-mask],[],[Mask matching reference model],[templates_original/template_T2w_180_pad_renorm_clamp_mask.mnc])
...
_arg_model="templates_original/template_T2w_180_pad_renorm_clamp.mnc"
_arg_model_mask="templates_original/template_T2w_180_pad_renorm_clamp_mask.mnc"
_arg_clobber="off"
_arg_verbose="on"
_arg_debug="off"
...
print_help()
{
  ...
  printf '\t%s\n' "-h, --help: Prints help"
  printf '\t%s\n' "--distance: Initial distance for correction (default: '70')"
  ...
  printf '\t%s\n' "--isostep: Isotropic resampling resolution in mm for N3 (default: '0.72')"
  printf '\t%s\n' "--big-misorientation, --no-big-misorientation: Input file is very misoriented (off by default)"
  printf '\t%s\n' "--model: Reference model used for determining FOV and mask (default: 'templates_original/template_T2w_180_pad_renorm_clamp.mnc')"
  printf '\t%s\n' "--model-mask: Mask matching reference model (default: 'templates_original/template_T2w_180_pad_renorm_clamp_mask.mnc')"
...
volpad -distance $(calc "int(${scale_factor}*5)") ${tmpdir}/norm_input.mnc ${tmpdir}/norm_input_padded.mnc
...
# Precorrect Round 1
hierarchical_N3 ${input} ${tmpdir}/fgmask.mnc ${tmpdir}/fgmask.mnc ${_arg_distance} $((_arg_levels - 2)) ${_arg_cycles} ${_arg_iters} ${tmpdir}/precorrect1.mnc ${tmpdir}/precorrect1_field.mnc
...
ImageMath 3 ${tmpdir}/fgweight.mnc GetLargestComponent ${tmpdir}/fgweight.mnc

if [[ ${_arg_big_misorientation} == "on" ]]; then

  ThresholdImage 3 ${tmpdir}/precorrect1_denoise.mnc ${tmpdir}/precorrect1_denoise_kmeans.mnc Kmeans 31
  ThresholdImage 3 ${MODEL} ${tmpdir}/model_kmeans.mnc Kmeans 31

  # Smooth using ~4 voxel fwhm and then downsample
  SmoothImage 3 ${tmpdir}/precorrect1_denoise.mnc 1.01911 ${tmpdir}/antsAI_subject.mnc 0
  SmoothImage 3 ${tmpdir}/model_kmeans.mnc 1.01911 ${tmpdir}/antsAI_template.mnc 0
  ResampleImage 3 ${tmpdir}/antsAI_subject.mnc ${tmpdir}/antsAI_subject.mnc 3x3x3 0 4 3
  ResampleImage 3 ${tmpdir}/antsAI_template.mnc ${tmpdir}/antsAI_template.mnc 3x3x3 0 4 3
  ...
  # antsApplyTransforms -d 3 -i ${MODELMASK} -r ${tmpdir}/antsAI_template.mnc -o ${tmpdir}/antsAI_template_mask.mnc -n GenericLabel

  # Rotate every 30 degrees through all orientations of space and then perform a rigid alignment
  antsAI -d 3 -v 1 \
    -m Mattes[ ${tmpdir}/antsAI_template.mnc,${tmpdir}/antsAI_subject.mnc,32,Random,0.75,1 ] \
  ...
  # Initialize the registration using the antsAI result
  antsRegistration_affine_SyN.sh --skip-nonlinear  --clobber  \
...
antsApplyTransforms -d 3 --verbose -n GenericLabel \
  -i ${MODELMASK} \
  -t [ ${tmpdir}/to_model_0_GenericAffine.xfm,1 ] \
  -o ${tmpdir}/subject_bigmask.mnc \
  -r ${tmpdir}/norm_input_extracted_repad.mnc

iMath 3 ${tmpdir}/subject_bigmask.mnc MD ${tmpdir}/subject_bigmask.mnc 2 1 ball 1

# Start again using the corrected FOV input
input=${tmpdir}/norm_input_extracted_repad.mnc

# Precorrect Round 2
# Note we don't do _arg_levels
hierarchical_N3 ${input} ${tmpdir}/fgweight.mnc ${tmpdir}/fgweight.mnc ${_arg_distance} $((_arg_levels - 2)) ${_arg_cycles} ${_arg_iters} \
...
antsRegistration_affine_SyN.sh --clobber --skip-nonlinear \
  ...
  --close \
  --fixed-mask ${MODELMASK} \
  --moving-mask ${tmpdir}/subject_bigmask.mnc \
...
cp -f ${tmpdir}/subject_mask.mnc ${tmpdir}/final_weight.mnc

#Switched 3rd argument from ${tmpdir}/fgmask.mnc
hierarchical_N3 ${tmpdir}/precorrect2.mnc ${tmpdir}/final_weight.mnc ${tmpdir}/final_weight.mnc \
