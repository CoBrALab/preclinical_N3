...
function make_syn_pyramid {
  ...
  slices=($(echo "${slices}" | sed 's/-//g' | tr 'x' '\n'))

  # This loop generates the registration scales and iterations by counting up, then everything needs to be reversed
  local octave=0
  local subsample=$(calc "2^${octave}")
  shrinks=(1)
  smooths=(0)
  iterations=(${final_iterations})
  while :; do
    # Increase the smoothing within the octave until the next octave's smoothing is met
    smooth_scale=0
    while (( subsample + smooth_scale <= subsample*2 )); do
      smooth=$(calc "(${subsample}+${smooth_scale})/(2*sqrt(2*log(2)))")
      shrinks+=("${subsample}")
      smooths+=("${smooth}")
      iterations+=($(calc ${final_iterations}*${subsample}))
      ((++smooth_scale))
    done

    # Increase the octave
    ((++octave))
    subsample=$(calc "2^${octave}")
    # Stop if number of voxels remaining is less then minimum
    nvoxels=$(round $(calc "${slices[0]}*${slices[1]}*${slices[2]}/(${subsample}^3)"))
    (( nvoxels > minimum_voxels )) || break

  done

  echo --shrink-factors $(join_by_reverse 'x' ${shrinks[@]}) \
    --smoothing-sigmas $(join_by_reverse 'x' ${smooths[@]}) \
    --convergence [ $(join_by_reverse 'x' ${iterations[@]}),${convergence},10 ]
}
...
function make_affine_pyramid {
  ...
  local minimum_voxels=8000
  if [[ ${close} == "on" ]]; then
    minimum_voxels=$(calc 4*${minimum_voxels})
  fi
  slices=($(echo "${slices}" | sed 's/-//g' | tr 'x' '\n'))

    # Should figure out a better way to keep doing more Rigid steps
  local transforms=('Affine[ 0.05 ]' 'Affine[ 0.1 ]' 'Similarity[ 0.1 ]' 'Rigid[ 0.1 ]' 'Rigid[ 0.1 ]' 'Rigid[ 0.1 ]' 'Rigid[ 0.1 ]')
  ...
  local subsample=$(calc "2^${octave}")
  while :; do
    # Highest Resolution, do an unsmoothed iteration
    if (( octave == 0 )); then
      shrinks=(1)
      smooths=(0)
      iterations=(${final_iterations})
    ...
    histogram_bins=$(round $(calc "64 / ${subsample}"))
    if (( histogram_bins < 32 )); then
      histogram_bins=32
    fi

    if [[ ${fixedmask} != "NOMASK" || ${movingmask} != "NOMASK" ]]; then
      if (( octave < 2 )); then
        regstage_mask="--masks [ ${fixedmask},${movingmask} ]"
      else
        if [[ ${repeat} == 0 ]]; then
          regstage_mask="--masks [ ${fixedmask},${movingmask} ]"
        else
          regstage_mask="--masks [ NOMASK,NOMASK ]"
        fi
      fi
    else
      regstage_mask=""
    ...
    # Append to the stack
    transform_stack+=("--transform ${transforms[$octave]} \
      --metric ${linear_metric}[ ${fixedfile1},${movingfile1},1,${histogram_bins},Regular,1,1 ] \
      --shrink-factors $(join_by_reverse 'x' ${shrinks[@]}) \
      --smoothing-sigmas $(join_by_reverse 'x' ${smooths[@]}) \
      --convergence [ $(join_by_reverse 'x' ${iterations[@]}),${convergence},10 ] \
      ${regstage_mask}")

    if [[ ${octave} == 2 && (${fixedmask} != "NOMASK" || ${movingmask} != "NOMASK") && ${repeat} == 0 ]]; then
      # If we have masks, we repeat the octave 2 again
      repeat=1
    else
      # Increase the octave
      ((++octave))
  ...
  stack_rev=()
  for ((i = ${#transform_stack[@]} - 1; i >= 0; i--)); do
    stack_rev+=("${transform_stack[i]}")
  done
...
slices=$(PrintHeader ${fixedfile1} 2)
info "Fixed image slices ${slices}"
...
if [[ ${_arg_skip_linear} == "off" ]]; then
  ...
  if [[ ${_arg_kmeans_transformed_linear} == "on" ]]; then
    ...
    ThresholdImage 3 "${movingfile1}" "${tmpdir}/moving_kmeans_1.h5" Kmeans 63
    ThresholdImage 3 "${fixedfile1}" "${tmpdir}/fixed_kmeans_1.h5" Kmeans 63
...
i=0
while ((i < ${#_arg_fixed[@]})); do
  syn_metric+=" --metric $(grep -o -E '^[a-zA-Z]+' <<<${_arg_syn_metric})[ ${_arg_fixed[${i}]},${_arg_moving[${i}]},${_arg_weights[$((i + 1))]},$(grep -o -E '[0-9]+' <<<${_arg_syn_metric}),Regular,1,1 ]"
  ((++i))
done
